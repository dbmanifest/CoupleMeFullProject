...
export default function EditModal({ style, onSave }) {
  ...
  const [imageSrc, setImageSrc] = useState(style?.imageSrc || ""); // Renamed description to value

  useEffect(() => {
    if (style) {
      setLabel(style.label || ""); // Set label from style
      setValue(style.value || ""); // Set value from style
      setImageSrc(style.imageSrc || ""); // Set value from style
    }
  }, [style]);
  ...
  const handleSave = async () => {
    const updatedCompanion = {
      ...
      imageSrc: imageSrc, // Updated key to description
    };
    const response = await fetch(`/api/bodytypes/${style.id}`, {
      method: "PUT",
    ...
    if (response.ok) {
      ...
      console.error("Failed to update bodytype");
    }
    ...
    >
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content position-relative">
          ...
          <h2 className="form-label font-bold text-lg h-9 flex items-center">Edit Bodytype</h2>
            <form>
              <div className="mb-3">
