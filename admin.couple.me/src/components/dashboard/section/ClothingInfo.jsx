...
import EditModal from "../modal/Clothing/EditModal";
import DeleteModal from "../modal/Clothing/DeleteModal";
import PostModal from "../modal/Clothing/PostModal";
...
export default function BodytypeInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/clothings?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.clothings);
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/clothings/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/clothings/${selectedStyle.id}`, {
        method: "DELETE",
      });
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold">Clothings</h2>

                  <CIcon
                    icon={cibAddthis}
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#clothingPostModal"
                    className=" text-black px-11 py-2 rounded-lg transition-colors hover:cursor-pointer hover:px-10"
                  />
                </div>
