...
import EditModal from "../modal/Breastsize/EditModal";
import DeleteModal from "../modal/Breastsize/DeleteModal";
import PostModal from "../modal/Breastsize/PostModal";
...
export default function BodytypeInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/breastsizes?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.breastsizes);
      setTotalPages(data.totalPages);
    } catch (error) {
      console.error("Error fetching breastsizes:", error);
    } finally {
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/breastsizes/${updatedStyle.id}`, {
        method: "PUT",
      ...
      console.error("Error updating breastsizes:", error);
    }
  };

  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/breastsizes/${selectedStyle.id}`, {
        method: "DELETE",
      });
      setStyles(styles.filter((c) => c.id !== selectedStyle.id));
    } catch (error) {
      console.error("Error deleting breastsizes:", error);
    }
  };

  const handlePostSave = async (newStyle) => {
    try {
      const response = await fetch("/api/breastsizes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error("Failed to create breastsize");
      }
      ...
      console.error("Error posting breastsize:", error);
    }
  };
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold" style={{width: "43rem"}}>Breast Sizes</h2>
                  ...
                  <CIcon
                    ...
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#breastsizePostModal"
                ...
                <div className="row flex items-center">
                  <div className="col-lg-12">
                    <input
                      type="text"
                      placeholder="Search Breastsizes"
                ...
                <table className="table-style3 table at-savesearch">
                  <thead className="t-head">
                    <tr>
                      ...
                      <th scope="col">Breast Size</th> {/* New Column */}
