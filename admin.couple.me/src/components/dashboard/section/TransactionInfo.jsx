...
export default function StyleInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/transactions?page=${currentPage}&limit=5&search=${encodeURIComponent(
          searchQuery
        )}`
      );
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/transactions/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/transactions/${selectedStyle.id}`, {
        method: "DELETE",
      });
