...
export default function MessageInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/messages?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/messages/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/messages/${selectedStyle.id}`, {
        method: "DELETE",
      });
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold flex-col" style={{width: "43rem"}}>Messages</h2>
                </div>
