...
import EditModal from "../modal/Occupation/EditModal";
import DeleteModal from "../modal/Occupation/DeleteModal";
import PostModal from "../modal/Occupation/PostModal";
...
export default function BodytypeInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/occupations?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.occupations);
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/occupations/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/occupations/${selectedStyle.id}`, {
        method: "DELETE",
      });
  ...
  const handlePostSave = async (newStyle) => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to create occupations");
      }
      ...
      console.error("Error posting occupations:", error);
    }
  };
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold">Occupations</h2>
                  ...
                  <CIcon
                    ...
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#occupationPostModal"
