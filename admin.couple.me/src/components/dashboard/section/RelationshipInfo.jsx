...
import EditModal from "../modal/Relationship/EditModal";
import DeleteModal from "../modal/Relationship/DeleteModal";
import PostModal from "../modal/Relationship/PostModal";
...
export default function BodytypeInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/relationships?page=${currentPage}&limit=5&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.relationships);
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/relationships/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/relationships/${selectedStyle.id}`, {
        method: "DELETE",
      });
  ...
  const handlePostSave = async (newStyle) => {
    try {
      ...
      console.error("Error posting relationship:", error);
    }
  };
              ...
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  ...
                  <CIcon
                    ...
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#relationshipPostModal"
                    className=" text-black px-3.5 py-2 rounded-lg transition-colors hover:cursor-pointer hover:px-2.5"
                  />
                </div>
                ...
                <table className="table-style3 table at-savesearch">
                  <thead className="t-head">
                    <tr>
                      ...
                      <th scope="col">Relationship</th> {/* New Column */}
