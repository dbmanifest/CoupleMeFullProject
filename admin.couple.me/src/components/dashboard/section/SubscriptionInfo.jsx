...
import EditModal from "../modal/Transaction/EditModal";
import DeleteModal from "../modal/Transaction/DeleteModal";
...
export default function SubscriptionInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/subscriptions?page=${currentPage}&limit=5&search=${encodeURIComponent(
          searchQuery
        )}`
      );
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/subscriptions/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/subscriptions/${selectedStyle.id}`, {
        method: "DELETE",
      });
  ...
  const handlePostSave = async (newStyle) => {
    try {
      ...
      console.error("Error posting subscriptions:", error);
    }
  };
              ...
              <div className="packages_table table-responsive">
                <table className="table-style3 table at-savesearch">
                  <thead className="t-head">
                    <tr>
                      <th scope="col">User Id</th> {/* New Column */}
                      <th scope="col">Stripe Customer Id</th>
                      <th scope="col">Stripe Subscription Id</th>
                      <th scope="col">Stripe Price Id</th>
                      <th scope="col">Stripe Current Period End</th>
                    </tr>
                  </thead>
