...
import OccupationCard from "../card/OccupationCard";
import { CIcon } from "@coreui/icons-react";
import { cibAddthis } from "@coreui/icons";
import EditModal from "../modal/Hobbies/EditModal";
import DeleteModal from "../modal/Hobbies/DeleteModal";
import PostModal from "../modal/Hobbies/PostModal";
...
export default function BodytypeInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/hobbies?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.hobbies);
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/hobbies/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/hobbies/${selectedStyle.id}`, {
        method: "DELETE",
      });
      setStyles(styles.filter((c) => c.id !== selectedStyle.id));
    } catch (error) {
      console.error("Error deleting hobbies:", error);
    }
  };
  ...
  const handlePostSave = async (newStyle) => {
    try {
      ...
      console.error("Error posting hobbies:", error);
    }
  };
              ...
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  ...
                  <CIcon
                    ...
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#hobbiesPostModal"
                    className=" text-black px-12 py-2 rounded-lg transition-colors hover:cursor-pointer hover:px-11"
                  />
                </div>
