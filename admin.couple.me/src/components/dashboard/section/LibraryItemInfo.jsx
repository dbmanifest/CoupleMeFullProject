...
export default function LibraryItemInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/libraryitems?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.personalities);
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/libraryitems/${updatedStyle.id}`, {
        method: "PUT",
      ...
      console.error("Error updating libraryitem:", error);
    }
  };

  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/libraryitems/${selectedStyle.id}`, {
        method: "DELETE",
      });
      setStyles(styles.filter((c) => c.id !== selectedStyle.id));
    } catch (error) {
      console.error("Error deleting libraryitem:", error);
    }
  };

  const handlePostSave = async (newStyle) => {
    try {
      const response = await fetch("/api/libraryitems", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(newStyle),
      });
      if (!response.ok) {
        throw new Error("Failed to create libraryitem");
      }
      const data = await response.json();
      setStyles([...styles, data]);
      fetchStyles();
    } catch (error) {
      console.error("Error posting libraryitem:", error);
    }
  };
  ...
  // console.log(styles, 'stylesssssssssssssssssss');
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold flex-col" style={{width: "43rem"}}>Library Items</h2>
                </div>
