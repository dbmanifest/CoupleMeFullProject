...
import EditModal from "../modal/Wallet/EditModal";
import DeleteModal from "../modal/Wallet/DeleteModal";
import PostModal from "../modal/Wallet/PostModal";
import { CIcon } from "@coreui/icons-react";
import { cibAddthis } from "@coreui/icons";
import Pagination1 from "@/components/section/Pagination1";
import Loader from "@/components/loader/Loader";
import WalletCard from "../card/WallerCard";
...
export default function WalletInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/wallet?page=${currentPage}&limit=5&search=${encodeURIComponent(
          searchQuery
        )}`
      );
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/wallet/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/wallet/${selectedStyle.id}`, {
        method: "DELETE",
      });
