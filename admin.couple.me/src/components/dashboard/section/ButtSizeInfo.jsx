...
import EditModal from "../modal/Buttsize/EditModal";
import DeleteModal from "../modal/Buttsize/DeleteModal";
import PostModal from "../modal/Buttsize/PostModal";
...
export default function BodytypeInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/buttsizes?page=${currentPage}&limit=10&search=${encodeURIComponent(
          searchQuery
        )}`
      );
      const data = await response.json();
      setStyles(data.buttsizes);
      setTotalPages(data.totalPages);
    } catch (error) {
      console.error("Error fetching buttsizes:", error);
    } finally {
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/buttsizes/${updatedStyle.id}`, {
        method: "PUT",
      ...
      console.error("Error updating buttsize:", error);
    }
  };

  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/buttsizes/${selectedStyle.id}`, {
        method: "DELETE",
      });
      setStyles(styles.filter((c) => c.id !== selectedStyle.id));
    } catch (error) {
      console.error("Error deleting buttsize:", error);
    }
  };

  const handlePostSave = async (newStyle) => {
    try {
      const response = await fetch("/api/buttsizes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error("Failed to create buttsizes");
      }
      ...
      console.error("Error posting buttsizes:", error);
    }
  };
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold" style={{width: "40rem"}}>Butt Sizes</h2>
                  ...
                  <CIcon
                    ...
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#buttsizePostModal"
                ...
                <table className="table-style3 table at-savesearch">
                  <thead className="t-head">
                    <tr>
                      ...
                      <th scope="col">Butt Size</th> {/* New Column */}
