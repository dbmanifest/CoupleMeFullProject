...
import EditModal from "../modal/Personality/EditModal";
import DeleteModal from "../modal/Personality/DeleteModal";
import PostModal from "../modal/Personality/PostModal";
...
export default function PersonalityInfo() {
  ...
  async function fetchStyles() {
    try {
      ...
      const response = await fetch(
        `/api/personality?page=${currentPage}&limit=5&search=${encodeURIComponent(
          searchQuery
        )}`
      );
  ...
  const handleSaveStyle = async (updatedStyle) => {
    try {
      const response = await fetch(`/api/personality/${updatedStyle.id}`, {
        method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`/api/personality/${selectedStyle.id}`, {
        method: "DELETE",
      });
  ...
  const handlePostSave = async (newStyle) => {
    try {
      ...
      console.error("Error posting personality:", error);
    }
  };
  ...
  return (
    <>
            ...
            <div className="dashboard_title_area items-center justify-between p-4 bg-white shadow-md rounded-lg">
              <div className="flex justify-between items-center">
                <div className="flex justify-between w-52 h-12 items-center">
                  <h2 className="text-xl font-semibold">Personalities</h2>

                  <CIcon
                    icon={cibAddthis}
                    size="sm"
                    data-bs-toggle="modal"
                    data-bs-target="#personalityPostModal"
                ...
                <table className="table-style3 table at-savesearch">
                  <thead className="t-head">
                    <tr>
                      ...
                      <th scope="col">Personality</th> {/* New Column */}
      ...
      <PostModal onSave={handlePostSave} /> {/* Pass handlePostSave */}
    </>
  );
}
