...
export async function GET(req) {
  try {
    ...
    const personalities = await prisma.message.findMany({
      where: {
        companionId: {
          contains: search,
        },
    ...
    const totalPersonalities = await prisma.message.count({
      where: {
        companionId: {
          contains: search,
        },
    ...
    console.log(personalities, 'personalitiesssssssssssssssssss');
...
export async function POST(req) {
  try {
    ...
    const newpersonality = await prisma.message.create({
      data: {
        ...body,
      },
    });
