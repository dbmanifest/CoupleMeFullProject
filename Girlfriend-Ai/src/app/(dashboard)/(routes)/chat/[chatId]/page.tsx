...
const ChatIdPage = async ({ params }: ChatIdPageProps) => {
  ...
  const formattedMessages = companion.messages.map(message => ({
    ...message,
    isImage: message.isImage || false,
  }));

  const formattedChatList: ChatListItem[] = chatList.map((chat) => ({
    id: chat.id,
    name: chat.name,
  ...
  return <ChatClient companion={{ ...companion, messages: formattedMessages }} chatList={formattedChatList} />;
};
